        ...
        match stream.read(&mut buffer) {
            Ok(bytes_read) => {
                ...
                match str::from_utf8(&[buffer[0]]) {
                    Ok("0") => {
                        if let Some((remote_name, remote_bind_address)) = str::from_utf8(&buffer[1..bytes_read]).unwrap().trim().split_once(char::is_whitespace){
                    ...
                    Ok("1") => {
                        if let Some(remote_name) = {app.lock().unwrap().contact_list_get_name_by_address(&remote_address)} {
                            println!("[{}]: {}", remote_name, str::from_utf8(&buffer[1..bytes_read]).unwrap());
                        }
                        else {
                            println!("[{}]: {}", remote_address, str::from_utf8(&buffer[1..bytes_read]).unwrap());
                        }
                    },
                    Ok("2") => {
