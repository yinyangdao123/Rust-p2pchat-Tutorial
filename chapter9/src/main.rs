...
use std::{env, thread};
use chapter9::app::App;

pub fn register_name(app:Arc<Mutex<App>>, mut stream:TcpStream, local_name:String) {
...
fn send_file(mut stream:TcpStream, path_str:String) {
    ...
    if fs::exists(&path).is_ok() && Path::is_file(&path) {
        ...
        stream.flush().unwrap();
        println!("Finished sending {}", path.file_name().unwrap().to_string_lossy());
    }
    else {
        println!("{} does not exists", path_str);
    }
}
            ...
            Ok(())=>{
                match str::from_utf8(&buffer_head) {
                            ...
                            Ok(count)=>{
                                if let Some((remote_name, remote_bind_address)) = str::from_utf8(&buffer[..count]).unwrap().trim().split_once(char::is_whitespace){
                                    if let Ok(remote_bind_address) = SocketAddr::from_str(remote_bind_address){
                                        println!("{}(bind_port:{}) as name {}", remote_address, remote_bind_address.port(), remote_name);
                        ...
                        match stream_bufreader.read(&mut buffer) {
                            Ok(count)=>{
                                if let Some(remote_name) = {app.lock().unwrap().contact_list_get_name_by_address(&remote_address)} {
                                    println!("[{}]: {}", remote_name, str::from_utf8(&buffer[..count]).unwrap());
                                }
                                else {
                                    println!("[{}]: {}", remote_address, str::from_utf8(&buffer[..count]).unwrap());
                                }
                            }
                            Err(_)=>{
                                println!("{} disconnected", remote_address);
                    ...
                    Ok("3") => {
                        ...
                        println!("We are receiving file: {}, size: {}", file_name, file_length);
...
fn main() -> std::io::Result<()> {
    ...
    loop{
        ...
        if input.len() == 0 {
            ...
            println!("Input message is too long!");
            continue;
        }
        ...
        match input_0{
            ...
            "lookup" => {
                ...
                if let Ok(remote_address) = SocketAddr::from_str(remote_address_or_name.replace(char::is_whitespace, "").as_str()) {
                    ...
                    println!("Please run [lookup ip:port|remote_name]");
                    continue;
                }
            }
            "file" => {
                if let Some((remote_address_or_name, path)) = input_1.split_once(char::is_whitespace) {
                    ...
                    if let Ok(remote_address) = SocketAddr::from_str(remote_address_or_name.replace(char::is_whitespace, "").as_str()) {
                        ...
                        println!("Please run [file ip:port|remote_name path]");
                        continue;
                    }
                }
            }
