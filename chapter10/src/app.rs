...
use crate::notice::{notice_and_prompt, Level};
        ...
        .for_each(|x|{
            ...
            if let Some(remote_name) = self.contact_list_get_name_by_address(remote_address) {
                if let Some(remote_bind_address) = self.contact_list_get_bind_address_by_name(&remote_name){
                    let notice = format!("{}(bind_port:{}) -> {}", remote_address, remote_bind_address.port(), remote_name);
                    notice_and_prompt(Some(&notice), None, Level::Info);
                }
                else {
                    let notice = format!("{}({}) -> {}", remote_address, "", remote_name);
                    notice_and_prompt(Some(&notice), None, Level::Info);
                }
            }
            else {
                let notice = format!("{}({}) -> {}",remote_address,"", "");
                notice_and_prompt(Some(&notice), None, Level::Info);
            }
        });
    }
